...
@section('content')
  <div class="py-8 px-8 rounded-xl bg-white shadow-login-box">
    <h3 class="flex items-center text-2xl mb-4">
      ...
      <span class="text-main group-hover:text-white">@lang('Subscriptions')</span>
    </h3>
        ...
        <table class="min-w-full divide-y divide-sh-border">
          ...
          @foreach($subscriptions as $subscription)
            <tr>
              ...
              <td class="px-4 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">
                <div class="inline-flex items-center gap-x-3">
                  <a href="{{route('client.orders.show',[$subscription->order?->id])}}" class="text-blue-500">{{$subscription->order?->code}}</a>
                </div>
              </td>
              <td class="px-4 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">{{price($subscription?->price) == 'Not Defined' ?  __('Not Defined') : price($subscription?->price)}}</td>
              ...
              <td class="px-4 py-4 text-sm text-gray-500 whitespace-nowrap">{{$subscription->start_at?->format('Y-m-d')}}</td>
              <td class="px-4 py-4 text-sm text-gray-500 whitespace-nowrap">{{$subscription->end_at?->format('Y-m-d')}}</td>

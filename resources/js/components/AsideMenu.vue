<template>
  <!-- absolute inset-y-0 start-0 lg:end-0 -->
  <div class="flex items-center">
    <button
      @click="toggleAsideMenu"
      type="button"
      class="inline-flex md:hidden items-center justify-center md:justify-start p-2 rounded text-main focus:outline-none focus:ring-0"
      aria-controls="mobile-menu"
      aria-expanded="false"
    >
      <span class="sr-only">Open main menu</span>

      <svg v-if="locale == 'ar'" id="openMenu" class="block h-6 w-6" width="24" height="16" viewBox="0 0 24 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M0.75 15.5H12V13H0.75V15.5ZM0.75 9.25H23.25V6.75H0.75V9.25ZM0.75 3H23.25V0.5H0.75V3Z" fill="#231F20" />
      </svg>

      <svg v-else id="openMenu" class="block h-6 w-6" width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M26.25 22.5H15V20H26.25V22.5ZM26.25 16.25H3.75V13.75H26.25V16.25ZM26.25 10H3.75V7.5H26.25V10Z" fill="#231F20" />
      </svg>
    </button>

    <div :class="{ block: isAsideMenuOpen, hidden: !isAsideMenuOpen }" class="fixed md:hidden inset-0 z-[45] backdrop-blur-sm bg-black/[0.3]"></div>
    <Transition :name="locale == 'ar' ? 'slide-left' : 'slide-right'">
      <aside :class="{ flex: isAsideMenuOpen, hidden: !isAsideMenuOpen }" class="fixed z-50 md:flex flex-col start-0 inset-y-0 w-72 bg-white">
        <div class="py-8 px-4 md:py-10 md:px-6 flex items-start justify-between">
          <a :href="`/${locale}/home`" class="inline-flex w-9/12 lg:w-full">
            <img class="w-full" :src="'/images/logo.png'" alt="logo" />
          </a>

          <button @click="toggleAsideMenu" type="button" class="inline-flex md:hidden items-center">
            <span class="w-7 h-7 text-main">
        ...
        <ul class="py-6 px-4 md:px-6 space-y-4">

          <li>
            <a :href="`/${locale}/client/profile`" class="inline-flex items-center text-xl w-full py-2.5 px-3 rounded-xl">
              <img class="w-10 h-10 me-4 rounded-xl" :src="'/images/user.png'" alt="" />
              <span class="text-main">عبدالعزيز احمد</span>
            </a>
          </li>
        </ul>

<!--        <h3 class="px-4 md:px-6 text-xl font-semibold">Dashboard</h3>-->

        <ul class="py-4 px-4 md:px-6 space-y-4">
          <li>
            <a :href="`/${locale}/home`" class="inline-flex items-center text-xl w-full py-2.5 px-3 rounded-xl">
              <svg class="w-10 h-10 me-4" width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
                    d="M6.7509 26.1457C7.49019 26.1495 8.20231 25.8673 8.73839 25.3582L8.9709 25.1332V40.5007C8.9709 41.6942 9.445 42.8388 10.2889 43.6827C11.1328 44.5266 12.2774 45.0007 13.4709 45.0007H34.5009C35.6944 45.0007 36.839 44.5266 37.6829 43.6827C38.5268 42.8388 39.0009 41.6942 39.0009 40.5007V25.1332L39.2334 25.3582C39.7779 25.8746 40.5021 26.1587 41.2525 26.1503C42.0029 26.1419 42.7205 25.8417 43.2534 25.3132C43.5238 25.0433 43.7376 24.7222 43.8823 24.3686C44.0271 24.015 44.0998 23.6361 44.0963 23.2541C44.0928 22.8721 44.0132 22.4946 43.862 22.1437C43.7108 21.7929 43.4912 21.4757 43.2159 21.2107L24.5259 3.21074C24.386 3.07556 24.1991 3 24.0046 3C23.8101 3 23.6232 3.07556 23.4834 3.21074L4.79339 21.2107C4.38519 21.6063 4.10354 22.1141 3.98407 22.6699C3.86461 23.2256 3.91272 23.8043 4.12231 24.3327C4.33189 24.861 4.69353 25.3154 5.16145 25.6381C5.62937 25.9609 6.18252 26.1375 6.7509 26.1457ZM21.0009 43.5007V33.7507H27.0009V43.5007H21.0009ZM37.5009 40.5007C37.5009 41.2964 37.1848 42.0595 36.6222 42.6221C36.0596 43.1847 35.2965 43.5007 34.5009 43.5007H28.5009V33.0007C28.5009 32.8018 28.4219 32.6111 28.2812 32.4704C28.1406 32.3298 27.9498 32.2507 27.7509 32.2507H20.2509C20.052 32.2507 19.8612 32.3298 19.7206 32.4704C19.5799 32.6111 19.5009 32.8018 19.5009 33.0007V43.5007H13.5009C12.7052 43.5007 11.9422 43.1847 11.3796 42.6221C10.817 42.0595 10.5009 41.2964 10.5009 40.5007V23.7007L24.0009 10.7857L37.5009 23.7007V40.5007ZM5.8284 22.2907L24.0009 4.79324L42.1734 22.2907C42.3045 22.4186 42.4086 22.5714 42.4798 22.7401C42.5509 22.9089 42.5875 23.0901 42.5875 23.2732C42.5875 23.4564 42.5509 23.6376 42.4798 23.8064C42.4086 23.9751 42.3045 24.1279 42.1734 24.2557C41.916 24.505 41.5717 24.6444 41.2134 24.6444C40.8551 24.6444 40.5108 24.505 40.2534 24.2557L24.5034 9.18824C24.3639 9.05528 24.1786 8.9811 23.9859 8.9811C23.7932 8.9811 23.6079 9.05528 23.4684 9.18824L7.7184 24.2557C7.46099 24.505 7.11672 24.6444 6.7584 24.6444C6.40008 24.6444 6.0558 24.505 5.7984 24.2557C5.66846 24.128 5.56526 23.9757 5.49481 23.8076C5.42436 23.6396 5.38808 23.4592 5.38808 23.277C5.38808 23.0948 5.42436 22.9144 5.49481 22.7464C5.56526 22.5783 5.66846 22.426 5.7984 22.2982L5.8284 22.2907Z"
                    fill="black"
                />
              </svg>
              <span class="text-main">{{locale == 'ar' ? 'الرئيسية' : 'Home'}}</span>
            </a>
          </li>
            <li>
              <a :href="`/${locale}/client/subscription`" class="inline-flex items-center text-xl w-full py-2.5 px-3 rounded-xl hover:bg-main group">
                <svg
                  class="w-10 h-10 me-4 fill-main group-hover:fill-white"
                  width="48"
                  height="48"
                  viewBox="0 0 48 48"
                  fill="none"
                ...
                <span class="text-main group-hover:text-white">{{locale == 'ar' ? 'المدفوعات' : 'Subscriptions'}}</span>
              </a>
            </li>
          <li>
            <a :href="`/${locale}/client/orders`" class="inline-flex items-center text-xl w-full py-2.5 px-3 rounded-xl hover:bg-main group">
              <svg class="w-10 h-10 me-4" width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
                  d="M36.1061 16.1205H43.1467L38.253 30.7905H19.0975L14.2033 16.1205L12.1614 10H4.85547"
                  stroke="black"
              ...
              <span class="text-main group-hover:text-white">{{locale == 'ar' ? 'الطلبات' : 'Orders'}}</span>
            </a>
          </li>
          <li>
            <a @click="logout" class="inline-flex cursor-pointer items-center text-xl w-full py-2.5 px-3 rounded-xl hover:bg-main group">
              <svg class="w-10 h-10 me-4  group-hover:fill-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" >
                <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0 0 13.5 3h-6a2.25 2.25 0 0 0-2.25 2.25v13.5A2.25 2.25 0 0 0 7.5 21h6a2.25 2.25 0 0 0 2.25-2.25V15M12 9l-3 3m0 0 3 3m-3-3h12.75" />
              </svg>

              <span class="text-main group-hover:text-white">{{locale == 'ar' ? 'تسجيل خروج' : 'Logout'}}</span>
            </a>
          </li>
        </ul>
      </aside>
...
<script>
import LangMenuMb from './LangMenuMb.vue'
...
export default {
  ...
  data() {
    return {
      isAsideMenuOpen: true, // Open by default on desktop screens
      isMobile: false,
    }
  },
  mounted() {
    this.isMobile = window.innerWidth <= 768 // Assuming 768px is the breakpoint for mobile screens
    if (this.isMobile) {
      // Close the aside menu by default on mobile screens
      this.isAsideMenuOpen = false
    }
    // Listen for window resize events to check if the screen size changes
  ...
  methods: {
    ...
    toggleAsideMenu() {
      // Toggle the aside menu only on mobile screens
      if (this.isMobile) {
        this.isAsideMenuOpen = !this.isAsideMenuOpen
      }
    },
    handleResize() {
      // Check if the screen size changes to determine if it's mobile or desktop
      this.isMobile = window.innerWidth <= 768
      if (this.isMobile) {
        // Close the aside menu when the screen size changes to mobile
        this.isAsideMenuOpen = false
      } else {
        // Open the aside menu when the screen size changes to desktop
        this.isAsideMenuOpen = true
      }
    },
  },
  beforeUnmount() {
